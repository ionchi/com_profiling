...
if __name__ == "__main__":
    ...
    collezione = df_communities.select('nodes', '_id.oid','com_id','cliques').collect()
    cliqueUtenti={}
    CliqueInComm={}
    for i in collezione:
        if(i.com_id!=None):
    ...
    for cliq in cliqueUtenti:
        ...
        for i in mappaApp:
            if mappaApp[i]==massimo:
                conteggio+=1
        dataSet.write(str(cliq)+","+str(len(cliqueUtenti[cliq]))+","+str(massimo)+","+str(conteggio)+","+str(CliqueInComm[cliq])+","+str(intrusi)+"\n")
