...
if __name__ == "__main__":
    ...
    dataSet_ACCORPATE.write("clique_id, interesse_id, presenze, numero_nodi \n")
    ...
    dataSet_ISOLATE.write("clique_id, interesse_id, presenze, numero_nodi \n")
    ...
    dataSet_SCARTATE.write("clique_id, interesse_id, presenze, numero_nodi,intrusi \n")
    ...
    for cliq in cliqueUtenti:
        ...
        if cliq in cliqueScartate:
            ...
            for key in mappaApp:
                dataSet_SCARTATE.write(str(cliq)+","+str(key)+","+str(mappaApp[key])+","+str(len(cliqueUtenti[cliq]))+"\n")
            ...
            if cliq in cliqueAccorpate:
                ...
                for key in mappaApp:
                    dataSet_ACCORPATE.write(
                        str(cliq) + "," + str(key) + "," + str(mappaApp[key]) + "," + str(len(cliqueUtenti[cliq])) + "\n")
            else:
                #clique singole
                interessiComm[cliq] = mappaApp
                for key in mappaApp:
                    dataSet_ISOLATE.write(
                        str(cliq) + "," + str(key) + "," + str(mappaApp[key]) + "," + str(len(cliqueUtenti[cliq])) + "\n")

        if intrusi>0:
            if cliq in cliqueScartate:
                dataSet_SCARTATE.write("\n" + "utenti nella clique senza interessi," + str(lista))
            else:
                if cliq in cliqueAccorpate:
                    dataSet_ACCORPATE.write("\n" + "utenti nella clique senza interessi," + str(lista))
                else:
